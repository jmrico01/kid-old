...
template <typename Allocator>
bool PsdFile::LoadLayerImageData(uint64 layerIndex, Allocator* allocator, ImageData* outImageData)
{
		...
		for (int r = 0; r < layerHeight; r++) {
			...
			while (true) {
				...
				if (header == -128) {
					...
					int repeats = 1 - header;
					for (int i = 0; i < repeats; i++) {
						uint64 ind = (pixelY * layerWidth + pixelX) * channels
							+ channelOffset;
						layerData[ind] = data;
						pixelX++;
					}
...
template <typename Allocator>
bool PsdFile::LoadLayerTextureGL(uint64 layerIndex, Allocator* allocator,
	GLint magFilter, GLint minFilter, GLint wrapS, GLint wrapT, TextureGL* outTextureGL)
{
	ImageData imageData;
	if (!LoadLayerImageData(layerIndex, allocator, &imageData)) {
